(function () {
  ...
  function getPerView(wrapperWidth) {
    if (wrapperWidth <= 1024) return 1;
    return 2;
  }

  function initSlider(el) {
    var track    = el.querySelector('.dx-fcs__track');
    var prevBtn  = el.querySelector('.dx-fcs__nav-btn--prev');
    var nextBtn  = el.querySelector('.dx-fcs__nav-btn--next');
    var dotsWrap = el.querySelector('.dx-fcs__dots');
    var wrapper  = el.querySelector('.dx-fcs__wrapper');

    if (!track) return;

    var cards = Array.prototype.slice.call(track.querySelectorAll('.dx-fcs__slide'));
    ...
    function renderDots() {
      ...
      for (var i = 0; i < dotCount; i++) {
        ...
        dot.type      = 'button';
        dot.className = 'dx-fcs__dot' + (i === currentIndex ? ' is-active' : '');
    ...
    function updateDots() {
      if (!dotsWrap) return;
      var dots = dotsWrap.querySelectorAll('.dx-fcs__dot');
  ...
  document.querySelectorAll('.dx-fcs').forEach(function (el) {
    initSlider(el);
  });
